...
public class IGasPeriodicElementFinder implements IPeriodicElementFinder {
    //IPeriodicElementFinder를 구현한 각 클래스에 맞게끔 isInstanceOf()를 오버라이딩하고 있다.
    //이를 통해 동일한 메소드호출 isInstaceOf()을 하더라도 각 객체에 따라서 다르게 동작하도록 다형성을 실현하였다.
    @Override
    public boolean isInstanceOf(PeriodicElement e){
        //매개변수 PeriodicElement형 e로 참조하는 인자로 전달된 인스턴스가 GasPeriodicElement인스턴스 이거나 GasPeriodicElement를 상속하는 인스턴스이면 true를, 아니면 false를 반환한다.
