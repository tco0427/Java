...
//import문을 통하여 java.util패키지의 Arrays,ArrayList,List클래스를 import한다.
...
public class PeriodicTable {
    ...
    //생성자
    //생성자를 통해 배열을전달받고 이를 elements가 참조할 경우
    //elements는 배열인스턴스(elements[0]...elements[n])을 참조하고
    //각각의 elements[0]...elements[n]들이 실제 PeriodicElement인스턴스를 참조하고 있는 것으로 볼 수 있다.서
    ...
    //즉 아래와 같은 경우 elements(PeriodicElement[])배열의 각 원소들(elements[0]...elmeent[n])이 PeriodicElement인스턴스를 참조하므로
    //Arrays.toString(elements)를 통해 PeriodicElement클래스에 정의되어 있는 toString()메소드를 호출한다.(배열의 원소 모두에 대해서)
    @Override
    ...
    //메소드들은 서로 매개변수의 타입에 차이가 있고 메소드의 이름과 반환형이 동일하므로 오버로딩관계라고 할 수 있다.


    //String symbol을 매개변수로 받는 find()메소드가 정의되어 있으므로 이 메소드를 오버로딩하려고할 때
    //매개변수로 String은 불가능하다 따라서 Element를 받는 find()메소드를 통해 오버로딩하였고
    ...
    public PeriodicElement find(Element element){
        for(PeriodicElement pe: elements){
            ...
            //Object에 정의된 equals()메소드를 오버라이딩한 String클래스의 equals()를 통해 문자열 내용을 비교하고 참일 경우 pe가 참조하는 PeriodicElement인스턴스 반환
    ...
    //매개변수로 전달받은 정수(int)는 원소번호를 뜻하고 이에 해당하는 PeriodicElement를 elements(PeriodicElement[])에서 찾아 반환한다.
    public PeriodicElement find(int number){
        //for-each를 통해 elements가 참조하는 PeriodicElement[]배열인스턴스에서 각 배열원소(객체에 대한 래퍼런스)가 참조하는 PeriodicElement인스턴스를
    ...
    //위의 오버로딩된 find메소드와 마찬가지로 for-each를 통해 배열을 돌며 해당하는 PeriodicElement인스턴스를 반환하고
    ...
    //인자로 전달된 state에 해당하는 PeriodicElement모두 반환
    public PeriodicElement[] find(State state){
        ...
        //다이아몬드 기호를 통한 타입 인자 생랴
        ArrayList<PeriodicElement> list=new ArrayList<>();
        ...
        //하위클래스 형식의 참조변수로 상위클래스의 인스턴스를 참조하는 방식은 옳지 않다.
    ...
    public void print(){
        for(PeriodicElement pe: elements){
            System.out.println("----------------------------------------------------------------------------------------");
            //pe.toString()을 통해 PeriodicElement에서 오버라이딩한 toString()이 호출된다.
